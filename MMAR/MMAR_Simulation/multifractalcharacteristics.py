...
class MultifractalCharacteristics(Graphs):
    ...
    ```python
    # This code is a Python implementation of multifractal analysis for financial time series.
    It is based on a class `MultifractalCharacteristics` that inherits from a `Graphs` class
    (which is not shown in the provided code). The goal is to study the multifractal characteristics
    of a financial time series, specifically time and price, using various mathematical and statistical techniques.

    ### Method `__init__`
    - Initializes the class and inherits attributes from the `Graphs` class.
    - Computes a series of characteristics such as partition functions, \( \tau \), \( f(\alpha) \), etc.

    ### Method `Partition_functions`
    This method calculates partition functions for different moments \( q \) and different time intervals \( \Delta t \).

    \[
    S_q(\Delta t) = \sum \left| X(t+\Delta t) - X(t) \right|^q
    \]

    - `alpha`: significance level for confidence intervals.
    - `qs`: list of moments \( q \) for which the function is to be calculated.
    - `deltas`: time intervals \( \Delta t \) for which the function is to be calculated.
    - `a, b, c`: limits and steps for \( q \) to be calculated.
    - Fits polynomial functions for each \( q \) using ordinary least squares.
    - Calculates confidence intervals for the slope and intercept of the fitted functions.

    ### Method `tauandalpha`
    This method calculates the functions \( \tau(q) \) and \( f(\alpha) \), which are fundamental in multifractal analysis.

    \[
    \tau(q) = \lim_{{\Delta t \to 0}} \frac{\log(S_q(\Delta t))}{\log(\Delta t)}
    \]
    \[
    f(\alpha) = q\alpha - \tau(q)
    \]
    - Uses splines to calculate derivatives and functions.
    - Calculates confidence intervals for \( \tau(q) \).

    ### Key Mathematical Aspects:
    1. It utilizes polynomial fitting using ordinary least squares to find the relationship between
       \( \log(S_q(\Delta t)) \) and \( \log(\Delta t) \).
    2. Calculates confidence intervals for the estimates, which is crucial for statistical inference.
    3. Uses the derivative of the spline function to compute \( f(\alpha) \) and \( \tau(q) \).

    This is a highly simplified calculation, but I hope it helps understand the basic mechanics of the code.
    ```
    ...
    def Partition_functions(self, alpha=0.05, graf1=True, result=False):
        ...
        ldeltas = len(
            deltas
        )  # Number of points for each partition "line" for different moments.
        ...
        partition_functions = [
            [
                np.sum((abs(self.X_t[deltat::deltat] - self.X_t[:-deltat:deltat])) ** q)
        ...
        adjustment_part_functions = [
            np.polyfit(
                np.log10(deltas),
                np.log10(partition_functions[i] / partition_functions[i][0]),
                1,
            )
            for i in range(npoints)
        ]
        coeficiente_normalizador = [
            adjustment_part_functions[i][1] for i in range(npoints)
        ]
        ...
        srs = [
            (
                np.sum(
                    (
                        np.log10(partition_functions[i] / partition_functions[i][0])
                        - np.poly1d(adjustment_part_functions[i])(np.log10(deltas))
                    )
                    ** 2
                )
                / (ldeltas - 2)
        ...
        intervalos_conf = [
            [
                ...
                * np.sqrt(
                    1 / ldeltas
                    + (deltas[j] - mdeltas) ** 2 / ((ldeltas - 1) * vardeltas)
                )
                for j in range(ldeltas)
            ]
            for i in range(npoints)
        ]
        ...
        intervalos_conf_pendiente = [
            t.ppf(alpha / 2, ldeltas - 2) * srs[i] / (np.sqrt(ldeltas - 1) * vardeltas)
            for i in range(npoints)
        ]
        ...
        if graf1:
            ...
            for i, q in enumerate(qslim):
                ax.plot(
                    np.log10(deltas),
                    np.poly1d([q / 2 - 1, 0])(np.log10(deltas)),
                    linestyle="-.",
                    color="black",
                )
                ax.plot(
            ...
            ax.set_ylabel(
                r"$log_{10}(\hat{S}_q(\Delta t))$", fontsize=15, fontweight="bold"
            )
            ...
            for i, q in enumerate(qslim):
                ax.plot(
                    np.log10(deltas),
                    np.log10(partition_functions[i]),
                    label=f"q = {q:.2f}",
                    color=colors[i % len(colors)],
                )
            ...
            ax.set_title(
                r"Unnormalized Partition Function $log_{10}(S_q(\Delta t))$ vs. $log(\Delta t)$ for different q values."
                + "\n Curves adjusted by OLS:",
                fontsize=16,
                fontweight="bold",
            )
    ...
    def tauandalpha(self, alpha=0.05, graf=True, results=False):
        ...
        tausposibles = [
            [
                np.log10(partition_functions[i][j] / partition_functions[i][0])
                / np.log10(deltas[j])
                for i in range(npoints)
            ]
            for j in range(1, ldeltas)
        ]
        splinesposibles = [UnivariateSpline(qs, taus) for taus in tausposibles]
        lengendreposbiles = [
            intervalo1 * splines.derivative()(intervalo1) - splines(intervalo1)
            for splines in splinesposibles
        ]
        ...
        if graf:
            ...
            for i in range(ldeltas - 1):
                ax1.plot(
                    intervalo1, splinesposibles[i](intervalo1), alpha=0.1, color="grey"
                )
            ax1.plot(intervalo1, spline(intervalo1), lw=2, color="blue")
            ax1.fill_between(
                qs,
                np.array(tau) - np.array(intervalos_conf_pendiente),
                np.array(tau) + np.array(intervalos_conf_pendiente),
                color="red",
            )
            ...
            for i in range(ldeltas - 1):
                ax2.plot(
                    intervalo1,
                    splinesposibles[i].derivative()(intervalo1),
                    alpha=0.1,
                    color="grey",
                )
    ...
    def Hurst(self, graf=False):
        ...
        adjustment_part_functions = [
            np.polyfit(np.log10(deltas), np.log10(partition_functions[i]), 1)
    ...
    def multifractal_measure_rand(
        ...
        def lognormal_base2(lambdas, varianza):
            normal_random_variable = np.random.normal(
                loc=lambdas, scale=np.sqrt(varianza)
            )
        ...
        for k in range(kmax):
            ...
            for _ in range(2**k):
                if masas1:
                    masas2 = False
                    m00 = (lambda x: x / (x + (1 / x)))(
                        2 ** -np.random.normal(lambdas, np.sqrt(varianza))
                    )
                    m11 = 1 - m00
                if masas2:
