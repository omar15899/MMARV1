...
class Binomial_Multifractal_Det:
    ...
    def f1(self, alpha):
        ...
        return -(alpha_max - alpha) / (alpha_max - alpha_min) * np.log2((alpha_max - alpha) / (alpha_max - alpha_min)) - \
               (alpha - alpha_min) / (alpha_max - alpha_min) * np.log2((alpha - alpha_min) / (alpha_max - alpha_min))
    ...
    def multifractal_measure_det(self, coef=False, graf=False):
        ...
        # Plot if specified
        if graf:
            intervals = [np.linspace(i * self.b ** (-self.kmax), (i + 1) * self.b ** (-self.kmax), self.m) for i in range(self.b ** self.kmax)]
            ...
            plt.title(rf"Deterministic multifractal measure normalized by a factor $2^k$" + f"\n at iteration k = {self.kmax}" 
                      + "\nwith " + fr"weights $m_0$ = {self.m00}, $m_1$ = {self.m11}")
            plt.xlim(0, 1)
    ...
    def Test0(self):
        ...
        x = [x - (bin_edges.tolist()[1] - bin_edges.tolist()[0]) / 2 for x in bin_edges.tolist()]
        ...
        axis.set_title('Experimental vs Theoretical f(alpha) \nvia the Histogram Method')
        ...
        print(f'The minimum and maximum alpha values are respectively: {-np.log2(self.m00)}, {-np.log2(self.m11)}. As we can see, they match the experimental values. \nThe maximum is {np.amax(y)} and the experimental alphas are {x[0]} and {x[-1]}')
    ...
    def Test1(self):
        ...
        for i, q in enumerate(interval):
            ax.plot([np.log10(2 ** ((i + 1))) for i in range(len(funciones_particion[i]))], 
                    np.log10(np.array(funciones_particion[i]) / funciones_particion[i][0]))
        
        # Set plot labels, title, and grid:
        ax.set_title(r"Normalized Partition Function $log_{10}(S_q(\Delta t))$ vs. $log(\Delta t)$ for different q values.", fontsize=16, fontweight='bold')
    ...
    def Test2(self):
        ...
        a, b = -5, 5
        npoints = 10000
        h = (b - a)/npoints  # Size of the interval between points; relevant for derivative calculations
        ...
        partition_functions = [np.array([np.sum(submeasure**q) for submeasure in measures]) for q in qs]
        
        # Normalize the partition functions relative to their first value
        partition_functions_norm = [np.log(func/func[0]) for func in partition_functions]
        
        # Calculate tau, which is defined as the slope of the log/log graph
        tau = [(func[1] - func[0])/(deltas[1] - deltas[0]) for func in partition_functions_norm]
        ...
        alpha_max = -np.log2(1 - a)
        x2 = np.linspace(alpha_min + 0.01, alpha_max - 0.01, 1000)
        ax3.plot(x2, self.f1(x2), color='red', label="Theoretical function", linestyle="dashdot")
        ax3.plot(derivative1, legendre1, lw=2, color='blue', label="Experimental function")
        ax3.set_ylim(0, 1.1)
    ...
    def Test3(self):
        ...
        print(f"Fit values for a function (K/np.sqrt(x))**W are K = {popt[0]} and W = {popt[1]}, with the respective fit errors: {perr}")
        ...
        axis[0].plot(xdata, theoretical_convergence(xdata), label="Theoretical Monte Carlo Error")
        axis[0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), fancybox=True, shadow=True, ncol=1)
        axis[0].set_title("Difference between experimental and actual value\n(relative error of the experimental maximum vs. the actual maximum = 1)\ncompared to the Monte Carlo simulation error", y=1.05)
        ...
        axis[1].set_title("Difference between the Monte Carlo error evolution and\nthe histogram error")
